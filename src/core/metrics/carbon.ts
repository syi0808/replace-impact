...
export const CARBON_EQUIVALENTS = {
  meal: {
    ...
    kgCo2e: 1.5,
    source: "Clever Carbon; food footprints vary by ingredient, region, and preparation"
  },
  warmShower10Min: {
...
export function calculateCarbonKgCO2e(trafficBytes: number | null): number | null {
  if (trafficBytes === null) {
    return null;
  }

  const trafficAvoidedGB = trafficBytes / 1_000_000_000;
  const energyKWh = trafficAvoidedGB * carbonAssumptions.energyIntensityKWhPerGB;
  return energyKWh * carbonAssumptions.gridIntensityKgCO2ePerKWh;
}

export function calculateCarbonEquivalents(savedKgCo2e: number | null): CarbonEquivalentSummary {
  if (savedKgCo2e === null) {
    return {
      meals: null,
  ...
  return {
    ...
    avoidedDamageUsd: savedKgCo2e * CARBON_EQUIVALENTS.socialCostOfCarbon.usdPerKgCo2e
  };
}

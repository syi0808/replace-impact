...
import { calculatePerInstallDelta, calculatePeriodValue } from "../../core/metrics/traffic";
...
import { cleanPackageName, findDirectDependency, isValidPackageName } from "../../core/npm/resolvePackage";
...
export async function createImpactReport(
  ...
  const cleanedTo = cleanPackageName(to);
  ...
  const directDependency = findDirectDependency(rootPackage.latest, cleanedFrom);
  ...
  const beforeRange = directDependency?.kind === "dependency" ? directDependency.range : "latest";
  ...
  const trafficPerInstall = calculatePerInstallDelta(before.tarballBytes, after.tarballBytes);
  const filesPerInstall = calculatePerInstallDelta(before.fileCount, after.fileCount);
  const unpackedPerInstall = calculatePerInstallDelta(before.unpackedBytes, after.unpackedBytes);
  const monthlyTraffic = calculatePeriodValue(trafficPerInstall, downloads.monthly);
  ...
  return {
    ...
    downloads,
    metrics: {
      traffic: buildMetric("Potential package traffic avoided", trafficPerInstall, downloads, "bytes"),
      files: buildMetric("Files not unpacked", filesPerInstall, downloads, "files"),
      unpacked: buildMetric("Filesystem work avoided", unpackedPerInstall, downloads, "bytes"),
      reach: {
        label: "Potential direct npm install paths improved",
        perInstall: directDependency?.kind === "dependency" ? 1 : null,
        monthly: directDependency?.kind === "dependency" ? downloads.monthly : null,
        yearly: directDependency?.kind === "dependency" ? downloads.yearly : null,
        unit: "installs"
      },
      carbonMonthly: {
        ...
        yearly: calculateCarbonKgCO2e(calculatePeriodValue(trafficPerInstall, downloads.yearly)),
        unit: "kgCO2e"
      }
    },
    warnings,
